...
public class GiftDiscountTest {
    ...
    static Stream<Arguments> discountTestArguments() {
        return Stream.of(
                ...
                arguments(new Order(new VisitDate(1), new OrderMenu(Map.of("티본스테이크", 1))), 0),
                arguments(new Order(new VisitDate(2), new OrderMenu(Map.of("시저샐러드", 11))), 0),
                arguments(new Order(new VisitDate(3), new OrderMenu(Map.of("초코케이크", 1))), 0),
                arguments(new Order(new VisitDate(4), new OrderMenu(Map.of("레드와인", 1, "양송이수프", 1))), 0),
                arguments(new Order(new VisitDate(5), new OrderMenu(Map.of("샴페인", 4, "초코케이크", 1))), 0),
                arguments(new Order(new VisitDate(6), new OrderMenu(Map.of("제로콜라", 14, "바비큐립", 1))), 0),
                arguments(new Order(new VisitDate(7), new OrderMenu(Map.of("해산물파스타", 1, "크리스마스파스타", 2))), 0)
        );
    }
}
